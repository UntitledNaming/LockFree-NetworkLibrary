...
- 설계 의도: IOCP concurrent 값을 러닝 스레드 수로 지정해 동시에 깨어나는 워커 수를 조절하고, 세션 테이블을 고정 배열 + Lock-Free 인덱스 스택으로 관리해 세션 할당/반환을 동기화 객체없이 한다.
...
- 설계 의도: 완료 키에 세션 포인터를 넣어 이후 완료 처리에서 세션 조회 비용을 없앤다. 세션 ID에 인덱스를 실어 조회를 배열 인덱싱 한 번으로 끝낸다. RST를 날려서 소켓에 TIME_WAIT 상태를 남기지 않고 바로 정리해 논 페이지드 풀 증가를 피한다.
...
- 설계 의도: TCP 스트림 경계 문제를 링버퍼 + 헤더 길이 기반 조립으로 처리한다. Net은 코드·길이 2중 검증으로 조작 패킷을 차단한다. 체크섬은 인코딩 된 페이로드를 디코딩 했을 때 정합성 검증을 위해 사용한다.
...
- 설계 의도: 여러 스레드의 송신을 세션 큐로 모으고, 송신 플래그로 세션당 송신을 하나로 유지해 반복적인 커널 모드 진입을 제한한다.
...
- 설계 의도: 세션 단위 동기화 객체 없이 참조 카운트 + 릴리즈 플래그를 하나의 128비트 CAS로 묶어 "마지막 사용자가 안전하게 반납"을 보장한다.
- 관련 테스트: 장시간 접속/해제 반복.
...
- 처리 순서:
  1. `CMessage::Alloc`은 TLS 풀의 `Alloc`을 호출한다.
...
- 설계 의도: 스레드별 서브풀로 대부분의 할당/반납을 동기화 없이 처리하고, 버킷 단위로만 공용 자료구조를 오가 경합 빈도를 줄인다. 소유 ID로 잘못된 반납을 차단한다.
...
- 처리 순서:
  ...
  2. `GetDBData`로 계정 DB(MySQL)에서 ID·닉네임을 조회한다(클라이언트가 들고 온 토큰을 플랫폼과 통신하여 검증하는 과정을 대체함).
