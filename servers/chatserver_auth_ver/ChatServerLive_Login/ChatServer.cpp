...
ChatServer::ChatServer() : m_AuthEvent(INVALID_HANDLE_VALUE), m_AuthTPS(0), m_MonitorFlag(false), m_ReqChatMsgTPS(0), m_ReqLoginTPS(0), m_ReqMoveTPS(0), m_ReqTPS(0), m_ResChatMsgTPS(0),
m_ResLoginTPS(0), m_ResMoveTPS(0), m_UpdateFrame(-1), m_UpdateLoopTime(0), m_UpdateTPS(0), m_UserMaxCnt(0), m_pAuthJobQ(nullptr), m_pJobPool(nullptr), m_pMonitorClient(nullptr)
, m_pPDH(nullptr), m_pRedisClient(nullptr), m_pUpdateJobQ(nullptr), m_pUserPool(nullptr)
{
	WSADATA wsa;
...
BOOL ChatServer::RunServer()
{
	...
	Parser::st_Msg mornitor;
	parser.GetValue("MORNITOR_IP", &mornitor);

	Parser::st_Msg redisip;
	parser.GetValue("REDIS_IP", &redisip);

	std::wstring mornitorstr;
	...
	INT mornitorport;
	parser.GetValue("MORNITOR_PORT", &mornitorport);
...
#pragma endregion
	...
	Mem_Init(Usermax, UpdateFrame, redisip.s_ptr, redisport,(WCHAR*)mornitorstr.c_str(), mornitorport);
...
void ChatServer::StopServer()
{
	...
	CloseHandle(m_AuthEvent);
	CMessage::PoolDestroy();
}

void ChatServer::Mem_Init(INT userMAX, INT updateFrame, const CHAR* redisip, INT redisport,WCHAR* MonitorIp, INT MonitorPort)
{
...
void ChatServer::Thread_Create()
{
	...
	m_Auth = std::thread(&ChatServer::AuthThread, this);
}
...
void ChatServer::UserTimeOut()
{
	...
	for (; it != m_UserMap.end(); ++it)
	{
		...
		if (curTick - pUser->s_RecvTime >= TIMEOUT2)
		{
			...
			CNetServer::Disconnect(pUser->s_UniqID);
			pUser->s_TimeOut = 1;
...
void ChatServer::LoginProc(CMessage* pMessage, UINT64 sessionid)
{
	...
	AuthRequestJob(sessionid, pMessage);
}
...
void ChatServer::ChatMessageProc(CMessage* pMessage, UINT64 sessionid)
{
	...
	*pMessage >> MsgLen;//메세지 바이트 단위 길이
	...
	// 직렬화 버퍼 크기 이상의 MsgLen or MsgLen은 정상이나 직렬화 버퍼에 담긴 데이터가 없을 때 끊기
	if (pMessage->GetData((char*)Message, MsgLen) == 0)
	{
		Disconnect(sessionid);
		LOG(L"ChatServer", en_LOG_LEVEL::dfLOG_LEVEL_ERROR, L"ChatMessageRequest::GetData Error... / UniqID : %lld  / AccountNo : %lld ", sessionid, pUser->s_AccountNo);
		return;
	}
...
void ChatServer::RecvProc(UINT64 sessionID, CMessage* pMessage)
{
	...
	*pMessage >> type;
	...
	{
		Disconnect(sessionID);
		LOG(L"ChatServer", en_LOG_LEVEL::dfLOG_LEVEL_ERROR, L"UserRecvMsg::CMessage Flag Error...  / UniqID : %llu ", sessionID);
...
void ChatServer::AuthComProc(UINT64 sessionID, CMessage* pMessage)
{
	BYTE  Status;
	INT64 accountno;
	WCHAR ID[ID_MAX];
	...
	// 해당 세션ID가진 유저가 인증 스레드 거친 사이에 먼저 지워질 수 있음. 
	// 해당 세션ID는 아직 NonUser에 있으니 찾아서 없으면 그냥 리턴할 것임.
	itNon = m_NonUserMap.find(sessionID);
	if (itNon == m_NonUserMap.end())
	{
		CMessage::Free(pMessage);
...
void ChatServer::AuthReqProc(UINT64 sessionID, CMessage* pMessage)
{
	INT64 accountNo = *reinterpret_cast<INT64*>(pMessage->GetReadPos());

	m_pRedisClient->get(to_string(accountNo), [this, pMessage, sessionID](cpp_redis::reply& reply)
		{
			INT64 accountNo = *reinterpret_cast<INT64*>(pMessage->GetReadPos());
...
void ChatServer::AuthCompleteJob(UINT64 sessionID, CMessage* pMessage)
{
	JOB* job = m_pJobPool->Alloc();
	job->s_Id = sessionID;
	job->s_Type = en_AuthCom;
	job->s_ptr = pMessage;

	m_pUpdateJobQ->Enqueue(job);
}

void ChatServer::AuthRequestJob(UINT64 sessionID, CMessage* pMessage)
{
	JOB* authjob = m_pJobPool->Alloc();
	authjob->s_Id = sessionID;
	authjob->s_Type = en_AuthReq;
	authjob->s_ptr = pMessage;

	pMessage->AddRef();

	m_pAuthJobQ->Enqueue(authjob);
	SetEvent(m_AuthEvent);
}
...
void ChatServer::UpdateThread()
{
	...
	while (!esc)
	{
		...
		while (m_pUpdateJobQ->GetUseSize() > 0)
		{
			...
			case en_AuthCom:
				AuthComProc(job->s_Id, job->s_ptr);
				break;
			}
...
void ChatServer::MonitorThread()
{
	...
	LOG(L"ChatServer", en_LOG_LEVEL::dfLOG_LEVEL_SYSTEM, L"MonitorThread  Start... : %d ", GetCurrentThreadId());
	...
	LOG(L"ChatServer", en_LOG_LEVEL::dfLOG_LEVEL_SYSTEM, L"MonitorThread  End... : %d ", GetCurrentThreadId());
}
...
void ChatServer::AuthThread()
{
	...
	LOG(L"ChatServer", en_LOG_LEVEL::dfLOG_LEVEL_SYSTEM, L"AuthThread  Start... : %d ", GetCurrentThreadId());
	...
	while (!esc)
	{
		...
		while (m_pAuthJobQ->GetUseSize() > 0)
		{
			...
			{
			case en_AuthReq:
				AuthReqProc(job->s_Id, job->s_ptr);
				break;
	...
	LOG(L"ChatServer", en_LOG_LEVEL::dfLOG_LEVEL_SYSTEM, L"AuthThread  End... : %d ", GetCurrentThreadId());
}
